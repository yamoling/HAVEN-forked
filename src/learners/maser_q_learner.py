...
class MASERQLearner:
    def __init__(self, mac: BasicMAC, macro_mac, value_mac, scheme: dict[str, Any], logger, args):
        ...
        match obs_shape:
            case tuple() as input_shape:
                cnn, n_outputs = make_cnn(input_shape, [32, 64, 32], [3, 3, 3], [1, 1, 1])
            ...
            case int():
                self.distance = nn.Sequential(
                    nn.Linear(self.scheme["obs"]["vshape"], 128),
                    nn.ReLU(),
                    nn.Linear(128, args.n_actions),
                ).to(self.device)
            case other:
                raise ValueError(f"Invalid obs_shape: {other}")

    def train(self, batch: EpisodeBatch, macro_batch, t_env: int, episode_num: int):
        # Get the relevant quantities

        rewards: th.Tensor = batch["reward"][:, :-1]  # type: ignore
        actions: th.Tensor = batch["actions"][:, :-1]  # type: ignore
        observation: th.Tensor = batch["obs"][:, :-1]  # type: ignore
        terminated: th.Tensor = batch["terminated"][:, :-1].float()  # type: ignore
        mask: th.Tensor = batch["filled"][:, :-1].float()  # type: ignore
        mask[:, 1:] = mask[:, 1:] * (1 - terminated[:, :-1])  # type: ignore
        ...
        if self.mixer is not None:
            ...
            goal_target_max_qvals = self.target_mixer(
                target_ind_qvals, batch["state"][:, :-1]
            )  # target_ind_qvals / ddqn_qval_up same result
            # (bs,t,1)
        ...
        if self.goal == "maser":
            ...
            explore_q_target = th.ones(target_ind_q.shape) / target_ind_q.shape[-1]  # diviser par le nombre d'actions
            ...
            for i in range(batch.batch_size):
                ...
                for j in range(self.n_agents):
                    ...
                    obs1 = observation[i, :, j, :]
                    obs2 = observation[i, ddqn_qval_up_idx[i][j], j, :].unsqueeze(0)
                    dist_obs = self.distance.forward(obs1)
                    dist_og = self.distance.forward(obs2).squeeze(0)
            ...
            repeats = [1] * ddqn_up.dim()
            repeats[1] = batch.max_seq_length - 1
            ddqn_up = ddqn_up.repeat(*repeats)
            # ddqn_up = ddqn_up.repeat(1, observation.shape[1], 1, 1)
            batch_size, ep_length, n_agents, *rest = observation.shape
            observation = observation.reshape(-1, *rest)
            ddqn_up = ddqn_up.reshape(-1, *rest)
            d1 = self.distance.forward(observation)
            d2 = self.distance.forward(ddqn_up)
            d1 = d1.reshape(batch_size, ep_length, n_agents, -1)
            d2 = d2.reshape(batch_size, ep_length, n_agents, -1)
            reward_ddqn_up = d1 - d2

            intrinsic_reward_list = []
            for i in range(self.n_agents):
                intrinsic_reward_list.append(-th.norm(reward_ddqn_up[:, :, i, :], dim=2).reshape(batch.batch_size, ep_length))
            intrinsic_rewards_ind = th.stack(intrinsic_reward_list, dim=-1)

            intrinsic_rewards = th.zeros(rewards.shape).to(device=self.device)

            for i in range(self.n_agents):
                intrinsic_rewards += -th.norm(reward_ddqn_up[:, :, i, :], dim=2).reshape(batch.batch_size, ep_length, 1) / self.n_agents
        ...
        if self.goal == "maser":
            ...
            individual_targets = (
                y * rewards
                + self.lam * intrinsic_rewards_ind
                + self.args.gamma * (1 - terminated.repeat(1, 1, target_individual_qvals.shape[-1])) * target_individual_qvals
            )
